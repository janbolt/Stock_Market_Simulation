...
- First you need to run the following lines in the directory of the project:

        mkdir build
        cd build 
        cmake ..

- Then you can run the simulation.cpp file to generate the .csv file with all the outputs.
...
## Fund
The **fund** represents the total invested capital in euros (€) across all shares.
...
### Strategy 2: Conservative Investment
Allocations are made according to the volatility of the shares, investing more in stocks with lower volatility (based on their respective variance).
...
## Variance
**Variance** indicates the volatility of a stock over a one-year period.

---

## Expected Return
The **expected return** is the anticipated profit, expressed as a percentage, over one year.

---

## Initial Value
The **initial calue** of the stock when purchased.

---

## Time Step
The time step is the amount of days that the stock market was simulated. One year has around 252 trading days (depending on country).
...
- $\sigma^2$: variance per year
- $S_n$: current stock price
- $dt$: 1/ 252 -> assuming 252 trading days per year

The sources for this formula: 
- https://raw.githubusercontent.com/0xnu/sps_gbm_ef/refs/heads/main/README.md
